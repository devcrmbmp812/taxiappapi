    ...
    $provider = Provider::find($request->id);
		...
		if ($validator->fails())
		{
          ...
          $total = $semi_total;// * ($service_tax_details->value/100) +$semi_total;
	    		...
	    		//Charges Balancedriver//
	    		$settings = Settings::where('key', 'komisi')->first();
                $komisi = $settings->value;
        
	    		  $driverDeposit = Balancedriver::where('driver_id', $provider->id)->first();
                                                if ($driverDeposit) {
	                                                
	                                                
                                                    $balance2 = $driverDeposit->amount_earned - $driverDeposit->amount_spent;
                                                   
                                                    if ($balance2 > 0) {
                                                        if ($total > 0) {
	                                                        
	                                                         $ref_total = $total;
                                                                $ledger_temp = Balancedriver::find($driverDeposit->id);
                                                                $ledger_temp->amount_spent = ($ledger_temp->amount_spent+($total*(100-$komisi)/100));
                                                                //$ledger_temp->amount_spent = ($ledger_temp->amount_spent+($nilai));
                                                                $ledger_temp->save();
                                                                $total = 0;
                                                            }
                                                        }
