 ...
 public function add_provider_process(Request $request)
    {
        ...
        $email = $request->email;
        $mobile = $request->mobile;
        $saldodriver = $request->balance_driver;
        $gender = $request->gender;
        $picture = $request->file('picture');
        $address = $request->address;
        ...
        else
        {
              ...
              if($request->id == ''){
               ...
               if($provider) {
                    ...
                    if($request->has('balance_driver')) {

                        $balance_driver = Balancedriver::where('driver_id' , $provider->id)
                                                ->first();

                        if(!$balance_driver) {
                            $provider_balance = new Balancedriver;
                        } else {
                            $provider_balance = $balance_driver;
                        }

                        $provider_balance->driver_id = $provider->id;
                        $provider_balance->amount_earned = $saldodriver;
    ...
    public function edit_provider(Request $request)
    {
        ...
        if($check_provider_service)
            ...
            $provider_type = "";
        $service_types = ServiceType::all();
        $balanceDriver = Balancedriver::where('driver_id' , $request->id)->first();
        return view('admin.add-provider')->with('name', 'Edit Provider')->with('provider_type',$provider_type)->with('provider',$provider)->with('service_types',$service_types)->with('balance_driver',$balanceDriver)
                                  ->withPage('providers')->with('sub_page','view-provider');
    }
    ...
    public function add_document_process(Request $request)
    {
			...
			'document_demo1' => 'required|max:255',
			//'document_demo1' => 'required|max:255',
                    )
                );
            if($validator->fails())
        ...
        else
        {
            ...
            else
            {
                ...
                $document->demo1 = $request->document_demo1;
                $document->demo2 = $request->document_demo2;
                $document->save();
