            ...
            <div class="box box-widget widget-user-2">
                ...
                <div class="box-footer no-padding">
                    <ul class="nav nav-stacked">
                        ...
                        <li>
                            <a href="javascript:void(0);">{{ tr('provider_name') }}<span class="pull-right">{{$request->provider_first_name}}</span></a>
                        </li>
                        <li>
                            <a href="javascript:void(0);">{{ tr('total_time') }}<span class="pull-right"> {{$request->total_time}}</span></a>
                        </li>
                        <li>
                            <a href="javascript:void(0);">{{ tr('request_started') }}<span class="pull-right">{{ date('jS \of F Y h:i:s A', strtotime($request->start_time)) }} </span></a>
                        </li>
                        <li>
                            <a href="javascript:void(0);">{{ tr('request_ended') }} <span class="pull-right">{{ date('jS \of F Y h:i:s A', strtotime($request->end_time)) }}</span></a>
                        </li>
                        <li>
                            <a href="javascript:void(0);">{{ tr('base_price') }}<span class="pull-right">{{ get_currency_value($request->base_price) }}</span></a>
                        </li>
                        <li>
                            <a href="javascript:void(0);">{{ tr('time_price') }}<span class="pull-right">{{ get_currency_value($request->time_price) }}</span></a>
                        </li>
                        <li>
                            <a href="javascript:void(0);">{{ tr('tax_price') }} <span class="pull-right"> {{ get_currency_value($request->tax) }}</span>
                            </a>
                         </li>
                        <li>
                            <a href="javascript:void(0);">{{ tr('total_amount') }}<span class="pull-right"> {{ get_currency_value($request->total_amount) }}</span></a>
                        </li>
                         <li>
                            <a href="javascript:void(0);">{{ tr('address') }}<span class="pull-right">{{$request->request_address}}</span></a>
                        </li>
                    </ul>
                </div>
